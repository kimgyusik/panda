								...
								<div class="col-lg-10 offset-lg-1">
									<div class="cart_container" style="width: 115%;">
										...
										<c:if test="${!empty list}">
											...
											<div class="tabmenu">
												<ul>
													<li id="displayAll" class="tab"><input type="radio"
														checked name="tabmenu" id="tabmenu"> <label
														class="categoryMenu" for="tabmenu">전체</label></li>
													<c:forEach items="${ category }" var="c" varStatus="status">
														<c:if test="${ status.count eq 5}">
															<li id="displayAll" class="tab"></li>
														</c:if>
														<li id="tab${fn:substring(c,0,1)}" class="tab"><input
															type="radio" name="tabmenu" id="tabmenu${c}"> <label
															class="categoryMenu" for="tabmenu${c}">${c}</label></li>
											...
											<div style="margin-top: 50px;">
												...
												<table>
													<c:forEach items="${ list }" var="g">
														...
														<tr id="${fn:substring(g.category2,0,1)}"
															class="contentsList" height="200px;">
															<c:url value="pDetailView.do" var="product">
																<c:param name="pId" value="${g.pId }" />
															</c:url>
															<td class="ggimTd1"><a href="${ product }"><img
																	class="ggimImg"
																	src="resources/product_uploadFiles/${g.paChangeName }"></a>
															</td>
															<td class="ggimTd2"><span style="color: gray;"><fmt:formatDate
																		value="${g.addDate}" pattern="yyyy. MM. dd" /></span><br>
															<br>
																[&nbsp;&nbsp;${g.category2}&nbsp;&nbsp;>&nbsp;&nbsp;${g.category}&nbsp;&nbsp;]
																<br> <span
																style="display: inline-block; font-size: 15px; height: 30px;"><a
																	href="${ product }"><b>${g.pName}</b></a></span><br>
																<p style="color: black;">
																	<fmt:formatNumber type="number" maxFractionDigits="3"
																		value="${g.price}" />
																	원 ~
																</p> <span style="color: gray;">${g.storeName }</span></td>
															<td width="250px;">
																<button class="addCart" onclick="addCart(${g.pId})">Add
																	to Cart</button>
															</td>
															<td style="vertical-align: top; padding: 10px;"><span
																class="cancle" onclick="cancle(${g.pId});">X</span></td>
														</tr>
													</c:forEach>
												</table>
										...
										<c:if test="${empty list}">
											<div style="text-align: center;">
												...
												<br>
												<br>
												<br> <img src="resources/images/pandaImage.jpg"
													width="100px;"> <br>현재 찜한 상품이 없습니다.
											</div>
										</c:if>
